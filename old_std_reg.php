...
<script src="include/js/jquery-1.10.1.min.js"></script>
...
function highlight(course){
	
	var chVal = $(course).prop('checked');
	if (chVal)
		$(course).parent().parent().css( "background-color", "#F5F5DC");
	else
		$(course).parent().parent().css( "background-color", "white");	
		

}
function showHideRegistable(t){
	
	var currentText = $(t).text();
	if (currentText == 'Show all courses')
		$(t).text('show only registable courses');
	else
		$(t).text('Show all courses');
		...
		<table cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
			...
			<tr height="22">		
				<td colspan='4' style="width:100%;padding: 0px 6px;">
					<input type="hidden" name="from_page" value="/SVUIS/old_std_reg.php" />
...
<font style='font-size:11px' color=red><center>page in 0.0027740001678467 seconds</center></font>
...
function chUnchInstallment(t){
	if($(t).prop("checked"))
		$('input[name="installmentthesis"]').attr('onClick','setUnsetCR(this);newCalcTotal();');
		
	else{
		$('input[name="installmentthesis"]').prop('checked', false);
		$('input[name="installmentthesis"]').attr('onClick','this.checked = false; setUnsetCR(this);newCalcTotal();');
	}
}		 

function setUnsetCR(t){
	
	if($(t).prop('checked'))
		$('input[name="correctionReason"]').val('installment Thesis..');
	else
		$('input[name="correctionReason"]').val('');
		
	
	
}

function applyEngRules(t){
	
	var chVal = $(t).prop('checked');
	var engCourId = getcounterPartEngCour(parseInt($(t).val()));
	engCourId = ''+engCourId+',36';	
	if ((engCourId != -1) && (chVal) ){
		$('input[name="c[]"]').each(
				function(i){
					if ( engCourId == $(this).val() ){
						$(this).prop('checked', !chVal);	
						highlight(this);
					}
...
function getcounterPartEngCour(engCourId){
...
function checkEDUSecondSeme(semeId){
	if (semeId >= 127 && semeId <= 142)
		return true;
	return false;
}


function getComplementaryHNDPro(projId){
...
function getSemesterId(cour){
	//232,36
	var cs = $(cour).val();
	var n = cs.indexOf(",");
	var semeId =  cs.substring(n+1,cs.length);
	if (semeId == 36)
		return -1;
	return semeId;
...
function getCourseId(courVal){
	//232,36
	var cs = $(courVal).val();
	var n = cs.indexOf(",");
	var courId =  cs.substring(0,n);
	return courId;

}

function applyBLProRules(t){
	var gi = parseInt($(t).next().next().next().next().next().val());
	if(gi != -1){
		var courState = $(t).next().next().next().val();
		if (courState == -1 ){
			$('input[name="c[]"]').each(
				function(i){
					var state = $(this).next().next().next().val();
					var cgi = parseInt($(this).next().next().next().next().next().val());
					if ( cgi != -1 && state == -1 && cgi != gi ){
...
function applyHNDProRules(t){
	
	var chVal = false;
	var courSemeId = getSemesterId(t);
	
	if (courSemeId != -1 ){
		
		var chVal = $(t).prop("checked");
		var courId = parseInt(getCourseId(t));
		var cc = getComplementaryHNDPro(courId);
		
		if (cc != -1){
			var comCour = $('input[value="'+cc+','+courSemeId+'"]');
			if(comCour.length != 0){
				comCour.prop('checked', chVal);
				highlight(comCour);
		...
		$('input[name="c[]"]').each(
				function(i){
					var state = $(this).next().next().next().val();
					var cs = getSemesterId(this);
					if ( courSemeId == cs && state == -1){
						$(this).prop('checked', chVal); 
						highlight(this);
					}
...
function regChecker () {
	...
	this.validate = function() {
    	...
    	switch (this.progId){
	    	case 2:
		    	return this.checkISErules();
		    	break;
			case 53:			
		    	return this.checkITErules();
		    	break;
	    	case 4: 
		    	return this.checkAHNDrules();
		    	break;
	    	case 6:
	    		return this.checkEHNDrules();
		    	break;
	    	case 8:	
	    	case 32: 	
	    	case 33:
	    	
	    		return this.checkBITrules();
		    	break;
	    	case 14:
	    		return this.checkBSCErules();
		    	break;
	    	case 27:
	    		return this.checkBMCrules();
		    	break;
	    	case 17:
	    	case 18:
	    	case 19:
			...
			case 78:
			case 79:
		   	case 23: if (this.checkThesisRules())
			...
			case 71:
			case 75:
			case 3: return this.checkMinMax(); 	
		   	default : return true;
    	}
	...
	this.getCourId = function (course){
		
		var cs = $(course).val();
		var n = cs.indexOf(",");
		var cid =  cs.substring(0,n);
		
		return cid;
	}
	
	this.reCalculateMin = function (){
		
		var n = this.numOfallCourOffered_notEng();
		var that = this;
		var min = parseInt($('#minimum').text());
		if(n < min ){
			$('#minimum').text(n);
			that.setMin(n);
		}
		...
		/*if(that.isMaster())	{
			...
			$('input[name="c[]"]').each(
	    	    	function(i){
	    	    		var courId = that.getCourId(this);
	    	    		var status = $(this).next().next().next().val();
		   	    		if(that.isThesis(courId) && status == -1 && that.stdRTerm > 2){
	    					$('#minimum').text(1);
							that.minCourReg = 1;
							return;
						}
	...
	this.setMin = function(newVal){
		this.minCourReg = newVal;
	}
	...
	this.isEngDepen = function(courId){
		switch (courId){
	    	case '171':
			case '172':
			case '173':
	...
	this.getSemeId = function (course){
    ...
    this.isIntEngCour = function (engCourId){
		if(engCourId >= 552 && engCourId <=556)
    		return true;
		return false;
	}

    this.isNorEngCour = function (engCourId){
		if(engCourId >= 232 && engCourId <=236)
    		return true;
		return false;
	}

    this.isNotCountedCour = function (courId){
    	if(courId == 544 || courId == 543 || courId == 542 || courId == 541 || courId == 202 || courId == 103)
    		return true;
		return false;
	}
	
    this.getCourCount = function (courId){
		if(this.isIntEngCour(courId))
    		return 2;
		if(this.isNotCountedCour(courId))
    		return 0;
		return 1;
		
	}

	this.numOfallCourOffered_notEng = function() {
    	
    	var nbCourses = 0;
    	var that = this;
    	$('input[name="c[]"]').each(
	    function(i){
	    		var courId = parseInt(that.getCourId(this));
		    	if ( (!that.isNotCountedCour(courId)) && (!that.isNorEngCour(courId)) && ((!that.isIntEngCour(courId))) )
	    			nbCourses++;
		});
	...
	this.isEngCourReg = function() {
	    ...
	    function(i){
	    		if ( $(this).prop("checked") ){
		    		var intensive = that.isIntEngCour(parseInt(that.getCourId(this)));
			    	var normal = that.isNorEngCour(parseInt(that.getCourId(this)));
		    		if (intensive || normal ){
		    			check = true;
		    			return true;
		    		}	
	...
	this.isIntEngCourReg = function() {
	    ...
	    function(i){
	    	    if ( $(this).prop("checked") ){
					var courId = parseInt(that.getCourId(this));
			    	var intensive = that.isIntEngCour(courId);
			    	if (intensive){
		    			check = true;
		    			return true;
					}
    ...
    this.numOfallCourSelected = function() {
	    ...
	    function(i){
	    		if ( $(this).prop("checked") ){
	    			var State = $(this).next().next().next().val();
	    			var SCState = $(this).next().next().next().next().next().next().val();
	    			if (!(State == 19 || State == 8 || SCState == -2)){
						var courId = parseInt(that.getCourId(this));
						nbCourses = nbCourses + that.getCourCount(courId);
					}
	 ...
	 this.numOfallOptionalEduCourSelected = function() {	      	
    	var nbbCourses = 0;
    	var that = this;
    	$('input[name="c[]"]').each
		(
	      function(i){
						var State = $(this).next().next().next().val();
						var courId = parseInt(that.getCourId(this));
						 //alert( 'courId:' + courId + ' State: ' + State +' -- SCState: ' + SCState   );						 						
						if ( (State==1) && (courId == 1078 || courId == 1079 || courId == 1080  || courId == 1081 || courId ==  1111 || courId ==  1113 || courId ==  1112 || courId == 1114  ))
						nbbCourses = nbbCourses + 1;
						if ( $(this).prop("checked") )
						{
							var courId = parseInt(that.getCourId(this));
							if (courId == 1078 || courId == 1079 || courId == 1080  || courId == 1081 || courId ==  1111 || courId ==  1113 || courId ==  1112 || courId == 1114  )
							nbbCourses = nbbCourses + 1;
						}
    ...
    /*this.numOfallSpecCourSelected = function() {
    ...
    this.numOfNewCourSelected = function() {
    	var nbCourses = 0;
    	var that = this;
    	$('input[name="c[]"]').each(
    	function(i){
	...
	this.checkCredit = function ()
	{
    	...
    	var totalNewCredit = 0;
    	var totalFailedCredit = 0;
    	var overAllTotals = 0;

    	$('input[name="c[]"]').each(
				function(){
					var creditValue = parseInt($(this).next().next().next().next().val());
					var State = parseInt($(this).next().next().next().val());
					var SCState = parseInt($(this).next().next().next().next().next().next().val());
					overAllTotals += creditValue;
					if ( $(this).prop("checked") ){
						if (!(State == 19 || State == 8 || SCState == -2)){	
							if (State == 2) 
		    					totalFailedCredit = totalFailedCredit+creditValue;
		    				else
		    					totalNewCredit = totalNewCredit+ creditValue;
		    			}		
					}
				}
		);

		var totalReg = totalFailedCredit + totalNewCredit;
		
		var min = this.minCredit;
			
		if (overAllTotals < this.minCredit)
			min = overAllTotals;
			
		
		//alert('totalFailedCredit='+totalFailedCredit+' totalNewCredit='+totalNewCredit); 	
		var checkNewCCredit = totalNewCredit <= this.maxNewCredit;
		var totalCredits = totalReg ;
		
		var intEngCase = false;
		if(this.isIntEngCourReg())
			if((totalCredits + 3) > this.maxCredit)
				intEngCase = true;
				
			
		
		var checkTotalCCredit =  ((totalCredits <= this.maxCredit) && (totalReg >= min  ));
		
		if (checkNewCCredit && checkTotalCCredit && (!intEngCase) || (this.isF13Drained) )
			return true;
		else{
			alert("يجب أن يكون عدد الساعات المعتمدة للمقررات الجديدة المسجلة أقل من "+this.maxNewCredit
			      +"\n \nو عدد الساعات المعتمدة لجميع المقررات المسجلة  بين "+this.minCredit+" و "+this.maxCredit
			      );
			return false;
		}
	 }

	this.checkISEprojects = function()
	{
		var numOfProjectreg = 0;
		var that = this;
		$('input[name="c[]"]').each(
			function(){		
				var courId = parseInt(that.getCourId(this));
				if($(this).prop("checked") && (courId == 79 || courId == 84) )
					numOfProjectreg += 1;
			}
		);
		if (numOfProjectreg == 2){
			alert('لا تستطيع التسجيل على المشروعين معاً');
			return false;	
		}
    ...
    this.checkISErules = function(){
    	
    	
    	if (this.checkCredit()){
        	if (this.checkISEprojects()) {
        		if(confirm('يرجى الانتباه إلى أنكم لا تستطيعون تعديل معلومات التسجيل حال الانتهاء من هذه الصفحة، هل أنتم متأكدون من المتابعة؟')) 
            		return true;	
            }
        }
    	return false;
    };
	this.checkCreditITE = function ()
	{
		...
		if (checkNewCCredit && checkTotalCCredit && (!intEngCase) )
			return true;
		else{
			alert("يجب أن يكون عدد الساعات المعتمدة للمقررات الجديدة المسجلة أقل من "+this.maxNewCredit
			      +"\n \nو عدد الساعات المعتمدة لجميع المقررات المسجلة  بين "+this.minCredit+" و "+this.maxCredit);
	...
	this.checkITErules = function(){
       	if (this.checkCreditITE())
        	return true;	
        return false;
    };
    ...
    this.checkIntCourIsSel = function(intCourId){
	...
	this.checkAHNDrules = function(){
    	
    	var check = true; 
    	var that = this;
    	var engRule =  that.checkIntCourIsSel(553);
    	
    	$('input[name="c[]"]').each(
    	    	function(i){
    				if ( $(this).prop("checked") ){
    					var State = $(this).next().next().next().val();
						var semeId =  that.getSemeId(this);
   					  	if (State == -1)
    						if((semeId == 26) && (!engRule) && (that.stdTerm >= 16)){
    							alert("يجب التسجيل على مستوى الانكليزي المكثف في حال سجلت على مقرر من الفصل الرابع");
    							check = false;
    				    		return false;
        					}
   	 	 ...
   	 	 if (check)
    	 	return that.checkMinMax();
 	 	 return false;
    };
    
    
    this.checkEHNDrules = function(){
        
    	var check = true;
    	var that = this;
    	var engRule =  that.checkIntCourIsSel(554);
    	$('input[name="c[]"]').each(
    	    	function(i){
    				if ( $(this).prop("checked") ){
    					var State = $(this).next().next().next().val();
    					...
    					var semeId =  that.getSemeId(this);
    					if (State == -1)
    						if((semeId == 30) && (!engRule)&& (that.stdTerm >= 16)){
    							alert("يجب التسجيل على مستوى الانكليزي المكثف في حال سجلت على مقرر من الفصل الرابع");
    							check = false;
    				    		return false;
        					}
    ...
    this.checkThesisRules = function(){
        
    	var check = true;
    	var that = this;
    	if(that.isThesisChecked()){
			var total_all = this.numOfallCourSelected();
			var total_new = this.numOfNewCourSelected();
			//alert('total_all:'+total_all  ) ;			
			//alert ('total_new:'+ total_new) ;
			var all_available_course = 0 ;
			
			$('input[name="c[]"]').each(
    	    	function(i){
    				if (  (!$(this).prop("disabled")) ){
    					all_available_course +=1;
    				}
    			}
    		);	
			//alert ('all_available_course:'+ all_available_course) ;
			 
			 if (all_available_course > 3)
			 {
			 
					 if (total_all != all_available_course)
					  {
							 alert('يجب تسجيل جميع المواد مع الرسالة');
							 check = false;
							 return;
					 }
					 ...
					 if (total_new > 0 )
					 {
						 alert('لا يسمح بتسجيل مواد جديدة مع الرسالة، يجب إنهاؤها قبل فصل الرسالة');
						 check = false;
						 return;
					}
			 }
			...
			$('input[name="c[]"]').each(
    	    	function(i){
    				if ( (!$(this).prop("checked")) && (!$(this).prop("disabled")) ){
    					alert('يجب أن تسجل على جيمع المقررات المتبقية في حال سجلت على الرسالة');
    					check = false;
    					return;
    				}
    			}
    ...
    this.checkMinMax = function(){		
    	...
    	var total = this.numOfallCourSelected();
		var numEduOptionalcourses = this.numOfallOptionalEduCourSelected();
		
		//alert(numEduOptionalcourses);
		if (numEduOptionalcourses > 2)
		{
			
    		alert("عدد المواد الاختيارية المسجل عليها أكبر من  2  ");
    		return false;
		}
    	
    	if (this.isF13Drained)
    		if (total > 0)
    			return true;
    		else{
				alert("يجب التسجيل على مادة واحدة على الاقل ");
				return false;
			}
    				
    	
    	var newC = this.numOfNewCourSelected();
    	/*var numOfSpecCours = this.numOfallSpecCourSelected();*/
    	
    	if(this.isMaster())
    		if(this.isEngCourReg()){
				total = total - 1;
				newC = newC - 1;
			}
    	
    	
		if (this.maxNewCourReg != 0){
			
        	if (newC > this.maxNewCourReg ){
				var HNDEngRule = false;
				//alert('newC='+newC+' isEngCourReg='+this.isEngCourReg());
        		if ((this.progId == 4 || this.progId == 6) && this.isEngCourReg())
        			HNDEngRule = true;

        		if (!HNDEngRule){
		    		alert("عدد المواد الجديدة المسجل عليها أكبر من الحد الأعلى ");
		    		return false;
        		}	
	    	}
    	}
    	
		var minCourse = parseInt($('#minimum').text());	
		var minTotal = total;
		if (this.isIntEngCourReg())
			minTotal = total - 1;
				
		if(minCourse > minTotal  ){
			
			alert("عدد المواد المسجل عليها أقل من الحد الأدنى");
				return false;
		}
		
		/*var totalWithoutSpecs = total - numOfSpecCours;*/
		
		if(total > this.maxCourReg && !this.isGraduating){
			 alert("عدد المواد المسجل عليها أكثر من الحد الأقصى .");
				 return false;
		}
    	...
    	$('input[name="c[]"]').each(
    	    	function(i){
    				if ( $(this).prop("checked") ){
						...
						var courID =  that.getCourId(this);
   					  	if (State == -1)
    						if((that.isEngDepen(courID)) && (!engRule) ){
    							alert("يجب التسجيل على مستوى الانكليزي المكثف ");
    							check = false;
    				    		return false;
        					}
    ...
    this.checkBMCrules = function(){
    	return  this.checkMinMax();
    	return true;
    };
}
// initialize javascript registering Class Checker...
var rC = new regChecker();
rC.reCalculateMin();

newCalcTotal();	
showHideRegistable($('#shRB'));
</script>
</html>